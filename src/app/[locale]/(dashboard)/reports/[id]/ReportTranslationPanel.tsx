...
export function ReportTranslationPanel({
  ...
  const t      = useTranslations('reportTranslation')
  ...
  function syncFromTranslation(tr: ReportTranslation) {
    setTranslation(tr)
    setEditFindings(tr.translatedFindings        ?? '')
    setEditImpression(tr.translatedImpression    ?? '')
    setEditRecs(tr.translatedRecommendations     ?? '')
    resetRejectForm()
    setError(null)
  }
      ...
      <div className="rounded-xl border border-violet-200 bg-white overflow-hidden">
        ...
        {!status && (
          <div className="px-6 py-10 text-center space-y-4">
            <p className="text-sm text-gray-500">{t('noTranslation')}</p>
            ...
            >
              {isPending ? <>{spinnerSvg} {t('generating')}</> : t('translateToFrench')}
            </button>
          </div>
        )}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 border-b border-gray-100">
              ...
              <div className="px-4 py-2.5 text-xs font-semibold text-violet-700 bg-violet-50/40">
                {t('translationColumn')}
                {status === 'draft' && (
                  <span className="ml-2 font-normal text-gray-400">{t('editable')}</span>
                )}
              </div>
            </div>
            ...
            <div className="divide-y divide-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="px-4 py-3 md:border-r border-gray-100 space-y-1">
                  <p className="text-xs font-medium text-gray-400 uppercase tracking-wider">{t('fieldFindings')}</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="px-4 py-3 md:border-r border-gray-100 space-y-1">
                  <p className="text-xs font-medium text-gray-400 uppercase tracking-wider">{t('fieldImpression')}</p>
              ...
              {(sourceRecommendations || editRecs || translation.translatedRecommendations) && (
                <div className="grid grid-cols-1 md:grid-cols-2">
                  <div className="px-4 py-3 md:border-r border-gray-100 space-y-1">
                    <p className="text-xs font-medium text-gray-400 uppercase tracking-wider">{t('fieldRecommendations')}</p>
            ...
            <div className="border-t border-gray-100 px-4 py-4 space-y-3">
              {status === 'approved' && translation.approvedAt && (
                <p className="text-xs text-gray-400">
                  {t('approvedAt', { date: formatDateTime(translation.approvedAt, locale) })}
                </p>
              )}
        ...
        {status === 'rejected' && translation && (
          <div className="p-5 space-y-4">
            <div className="rounded-lg bg-red-50 border border-red-200 px-3 py-2.5 text-xs text-red-800">
              ...
              {translation.rejectionReason && <span> {t('reason')}{translation.rejectionReason}</span>}
            </div>
            {translation.rejectedAt && (
              <p className="text-xs text-gray-400">
                {t('rejectedAt', { date: formatDateTime(translation.rejectedAt, locale) })}
              </p>
            )}
            {error && <p className="text-xs text-red-600">{error}</p>}
            <button
